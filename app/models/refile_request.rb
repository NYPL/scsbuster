...
class RefileRequest
  ...
  def assign_bearer
    begin
      ...
      if response.code == '200'
        ...
        self.bearer = nil
        CustomLogger.new("level" => "WARNING", "message" => "Bad response getting bearer token, response code is #{response.code}").log_message
      end
    rescue Exception => e
      CustomLogger.new("level" => "ERROR", "message" => "Error getting bearer #{e.backtrace}").log_message
      self.bearer = nil
    end
  end
  ...
  def post_refile
    ...
    if response.code == "200"
      ...
      CustomLogger.new("level" => "ERROR", "message" => "Error posting refile request to API. #{response.code} #{response.body}").log_message
      {}
    end
  end
